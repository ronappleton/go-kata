...
function renderCompileError(result) {
  el.runOutput.innerHTML = "";

  const banner = document.createElement("div");
  banner.className = "compile-error-banner";
  banner.innerHTML = `<strong>Compilation failed</strong> <span>(${result.duration_ms}ms)</span>`;
  el.runOutput.appendChild(banner);

  if (result.coach_hint) {
    const hint = document.createElement("p");
    hint.className = "coach-hint-line";
    hint.textContent = result.coach_hint;
    el.runOutput.appendChild(hint);
  }

  if (result.output_tail) {
    const pre = document.createElement("pre");
    pre.className = "compile-error-output";
    pre.textContent = result.output_tail;
    el.runOutput.appendChild(pre);
  }
...
function renderTestResults(result) {
  ...
  summary.className = result.passed ? "test-summary pass" : "test-summary fail";
  summary.innerHTML = `<strong>${result.passed ? "PASS" : "FAIL"}</strong> <span>${result.duration_ms}ms</span>`;
  el.runOutput.appendChild(summary);
  ...
  if (result.failed_tests && result.failed_tests.length > 0) {
    ...
    failList.className = "failed-test-list";
    result.failed_tests.forEach((name) => {
      const item = document.createElement("div");
      item.className = "failed-test-item";
      item.innerHTML = `<span class="test-icon fail-icon">✗</span> ${escapeHTML(name)}`;
      failList.appendChild(item);
    });
    el.runOutput.appendChild(failList);
  }
  ...
  if (result.output_tail) {
    ...
    summaryEl.textContent = "Raw output";
    details.appendChild(summaryEl);
    const pre = document.createElement("pre");
    pre.textContent = result.output_tail;
    details.appendChild(pre);
    el.runOutput.appendChild(details);
  }
